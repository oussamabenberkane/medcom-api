...
import com.oussamabenberkane.medcom.service.NotificationQueryService;
...
/**
 * REST controller for managing {@link com.oussamabenberkane.medcom.domain.Notification}.
 */
@RestController
...
public class NotificationResource {
    ...
    /**
     * {@code GET  /notifications/history} : Get notification history for the current authenticated user.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of notifications in body.
     */
    @GetMapping("/history")
    public ResponseEntity<List<NotificationDTO>> getNotificationHistory(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        String currentUserLogin = SecurityUtils.getCurrentUserLogin()
            .orElseThrow(() -> new AccessDeniedException("No authenticated user found"));

        LOG.debug("REST request to get notification history for user: {}", currentUserLogin);

        Page<Notification> page = notificationRepository.findByUserLoginOrderByCreatedAtDesc(currentUserLogin, pageable);
